...
session = tf.Session()
x = tf.placeholder(tf.float32, shape=[None, 96, 128, 2], name='x')
...
def create_convolutional_layer_tim(input, num_input_channels, conv_filter_size1, conv_filter_size2, num_filters):
    ...
    layer = tf.nn.conv2d(input=input, filter=[conv_filter_size1, conv_filter_size2, num_input_channels, num_filters], strides=[1, 1, 1, 1], padding='SAME')
    layer += biases
...
def create_convolutional_layer(input, num_input_channels, conv_filter_size, num_filters):
    ...
    biases = create_biases(num_filters)
    layer = tf.nn.conv2d(input=input, filter=[conv_filter_size, conv_filter_size, num_input_channels, num_filters], strides=[1, 1, 1, 1], padding='SAME')
    layer += biases
    layer = tf.nn.max_pool(value=layer, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    layer = tf.nn.relu(layer)
    return layer
...
def create_fc_layer(input, num_inputs, num_outputs, use_relu=True):
    ...
    layer = tf.matmul(input, [num_inputs, num_outputs]) + biases
    layer = tf.nn.relu(layer)
    return layer
...
layer_fc1 = create_fc_layer(input=layer_flat, num_inputs=layer_flat.get_shape()[1:4].num_elements(),
                            num_outputs=128, use_relu=True)

layer_fc2 = create_fc_layer(input=layer_fc1, num_inputs=128, num_outputs=num_classes, use_relu=False)
