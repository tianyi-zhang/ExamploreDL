...
class traverse_dic():
    ...
    def traverse_attribute(self, func: dict, traverse_flag: list, var_dic, return_flag=False, class_var_dic=None):
        ...
        if func['value']['_type'] == 'Attribute':
            return self.traverse_attribute(func['value'], traverse_flag, var_dic, return_flag, class_var_dic) + '.' + attr
        ...
        if dic[body_flag] is None:
            ...
            for body in dic[body_flag]:
                        ...
                        if key == "Call":
                            ...
                            for class_key in result['Class'].keys():
                                ...
                                if init_name in self.class_def[result['Class'][class_key]]['functions'].keys():
                                    function_body.append(self.class_def[result['Class'][class_key]]['functions'][init_name])
                        else:
                            var_dic[key] = result[key]
                elif body['_type'] == 'Call':
                    function_body.append(self.traverse_function_call(body, traverse_flag, var_dic, return_flag, class_var_dic))
                    # {'func': 'func_name', 'args': [], 'layer': '', 'body': [], 'lineno': 23}
                elif body['_type'] == 'For':
                    return_flag = True
                ...
                # TODO: add traverse_while
                    ...
                    function_body += self.traverse_with(body, body_flag, traverse_flag, var_dic, return_flag, class_var_dic)
                elif body['_type'] == 'Expr':
                    function_body.append(self.traverse_expr(body['value'], traverse_flag, var_dic, return_flag, class_var_dic))
                else:
                    pass
    ...
    def traverse_function_call(self, dic: dict, traverse_flag: list, var_dic, return_flag=False, class_var_dic=None):
        ...
        for kw in keywords:
            ...
            if kw['value']['_type'] == 'Call':
                kw_dic[kw_name] = self.traverse_function_call(kw['value'], traverse_flag, var_dic, return_flag, class_var_dic)
            elif kw['value']['_type'] == 'Attribute':
                kw_dic[kw_name] = self.traverse_attribute(kw['value'], traverse_flag, var_dic, return_flag, class_var_dic)
    ...
    def traverse_for(self, for_cell: dict, traverse_flag=None, var_dic=None, return_flag=True, class_var_dic=None):
        ...
        if for_cell['iter']['_type'] == 'Call' and 'id' in for_cell['iter']['func'].keys() and for_cell['iter']['func']['id'] == 'range':
            range_li = []
