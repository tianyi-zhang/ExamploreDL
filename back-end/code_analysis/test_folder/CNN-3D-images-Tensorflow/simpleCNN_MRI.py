...
h_conv1 = tf.nn.relu(h_conv1 + b_conv1)  # conv2d, ReLU(x_image * weight + bias)
...
h_conv2 =conv3d(h_pool1, W_conv2)
h_conv2 = tf.nn.relu(h_conv2 + b_conv2)  # conv2d, .ReLU(x_image * weight + bias)
...
out = tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=y_conv)
cross_entropy = tf.reduce_mean(out)
lr = 1e-3
keep_prob = 0.5
epochs = 100
batch_size = 20
train_step = tf.train.AdamOptimizer(lr)  # 1e-4
